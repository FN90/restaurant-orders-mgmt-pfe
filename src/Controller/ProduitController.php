...
class ProduitController extends AbstractController
{
     ...
     */
    public function add_to_cart(Request $request, SessionInterface $session)
    {
        $panier =  $session->get('panier', []);

        $id = $request->query->get("produit_id");
        $selectedIngredients = $request->query->get("selectedIngredients");

        if (!empty($panier[$id])) {
            $currentCount = count($panier[$id]);
            $panier[$id] = [
            ...
            $panier[$id] = [];
            $panier[$id]['ingredients'] = $selectedIngredients;
            $panier[$id]['count'] = 1;
        }
